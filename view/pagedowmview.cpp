...
int PageDowmView::process(struct CookEvent& cookEvent)
{
	...
	if (pfileManagerWindow->isEndPage()) {
		/* 已经是最后一页了，不需要处理 */
		delete textParser;
		return 0;
	}
	...
	/* 更新当前页数 */
	pfileManagerWindow->incCurPageCount();
	...
	while(1) {
		file = readdir(pDir);
		if (file == NULL) {
			pfileManagerWindow->setEndPageFlag(true);
			break;
		}
